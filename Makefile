...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/multisource.go
	rm -f go/multisource.go.bak
...
# not accept a path dependency, so tabnas-multisource is not published.
