...
fn version_looks_like_a_semver() {
    let parts: Vec<&str> = tabnas_multisource::VERSION.split('.').collect();
    assert_eq!(
        parts.len(),
        3,
        "VERSION is not x.y.z: {}",
        tabnas_multisource::VERSION
    );
    for part in parts {
        assert!(
...
fn version_matches_the_go_port() {
    let source = fs::read_to_string(repo_root().join("go").join("multisource.go"))
        .expect("go/multisource.go is readable");
    let declared = source
        .lines()
        .find_map(|line| line.trim().strip_prefix("const VERSION = \""))
        .and_then(|rest| rest.split('"').next())
        .expect("the Go port declares a VERSION");
    assert_eq!(
        declared,
        tabnas_multisource::VERSION,
        "go/multisource.go disagrees with VERSION"
    );
}
