...
a `// =>` comment, and `ts/test/doc-examples.test.ts` executes it. The Go
examples in `go/doc/guide.md` carry `// =>` comments too, and no Go test
executes them yet. A snippet that cannot be executed says why in one
