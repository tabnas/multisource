...
// Package main builds the C-ABI shared library: libtabnasmultisource.
//
// tabnas-clib-template: v4 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasmultisource.so ./clib
//
// multisource TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a multisource parser and returns a handle to it.
...
// tabnas_parse checks one input against the multisource grammar.
//
//export tabnas_parse
