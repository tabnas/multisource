...
// convenience Parse() rebuilds the (expensive) engine + multisource grammar on
// every call instead of reusing a cached instance. Building the grammar
// dominates a parse, so a rebuild-per-call Parse() is many times slower than
// reusing one MakeJsonic() instance.
//
// Only the default (no-options) Parse() path is cached, so this test exercises
// exactly that path: Parse(src) with no MultiSourceOptions.
//
