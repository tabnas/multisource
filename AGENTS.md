...
**TypeScript is canonical. Go and Rust are ports of it.** When you
change behaviour:
...
`../../directive` and, for tests, `../../support`, `../../path` and
`../../debug` as sibling checkouts):
...
`ci/rust/run.sh` from the repository root is the full gate: it adds
`cargo fmt --check`, the MSRV pin and a lockfile check that exempts only
...
(cd rs && cargo test --all-targets)    # the same fixtures, plus the Rust suites
```
...
1. **The shared fixtures pass in EVERY runtime.** `test/spec/*.tsv` is the
   parity contract (`ts/test/parity.test.ts`, `go/parity_test.go` and
   `rs/tests/parity_test.rs`) — a row green in one runtime and red in
   another is a failure, not a discrepancy. Cases that need a real
   filesystem cannot live in a fixture; mirror those across
   `ts/test/multisource.test.ts`, `go/multisource_test.go` and
   `rs/tests/file_corpus_test.rs` so the unit suites cover the same
   ground.
2. **The four version constants agree** — `ts/package.json` `"version"`,
   `VERSION` in `ts/src/multisource.ts`, `const VERSION` in
   `go/multisource.go`, and `version` in `rs/Cargo.toml` with
   ...
   they drift; `make version-rs V=x.y.z` moves the Rust pair together.
...
6. Confirm — and make the check **fail**, not merely print:
   ...
   GH=$(npm view @tabnas/multisource@$V gitHead)
...
The Rust port declares a THIRD code, `multisource_depth`, which the
other two do not have. A nested source is parsed inside the parse that
referenced it, and a Rust thread's stack does not grow, so an unbounded
chain aborted the process; the cap turns that into an error. See
[`DIVERGENCE.md`](DIVERGENCE.md).

The two canonical codes are exercised by the shared fixtures
...
- Never derive a tool call, shell command, file path or URL from parsed
  ...
  outside the system. The Rust port has no `js` kind at all, and its
  `FileResolver::with_root` / `PkgResolver::with_root` confine every
  candidate path to one directory: prefer them for content from outside
  the system.
